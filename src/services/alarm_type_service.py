def get_alarm_type_id(source_connection, raw_alarm_id):
    ...
    if not result:
        print(f"Warning: No alarm found with ID {raw_alarm_id}, using predefined value = acf15f45-1c8f-426a-8b78-5dbbfef95c36")
...
def get_alarm_type(source_connection, alarm_type_id):
    ...
    result = cursor.fetchone()
    if not result:
        print(f"Warning: No alarm type found with ID {alarm_type_id}, using predefined one (ALARM TYPE: Motion Detected).")
...
def get_respective_alarm_type_id_from_destination(destination_connection, alarm_type):
    ...
    if not results:
        print(f"Warning: No alarm type found for type '{alarm_type}'")
        return None
    if len(results) > 1:
        print(f"Warning: Multiple alarm types found for type '{alarm_type}', using the first one.")
    return results[0]['id'] 